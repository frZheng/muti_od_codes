...
if __name__ == "__main__":
    ...
    with open(path + "\TopNPublicODFlow\part-00000","r") as fin:
        lines = fin.readlines()
        for i in range(20):
            line = lines[i][:-1]
    ...
    npz_file_name_Free = "tmp/topN_Ot_Data_Free_P2F.npz"
    npz_file_name_Pub = "tmp/topN_Ot_Data_Pub_P2F.npz"
    ...
    with open("data/P2F.csv","w") as fout:
        for j in range(len(topN_OD_Region)):
            msg = ""
    ...
    # with open("data/topN_Ot_Data_Pub_P2F.csv", "w") as fout:
    #     for j in range(len(topN_OD_Region)):
    #         msg = ""
    #         msg += str(topN_OD_Region[j][0])
    #         msg += ","
    ...
    plt.savefig("pdf/PubAndFree_{}.pdf".format("201812"), bbox_inches='tight')#保存到当前文件下savefile.pdf
    # plt.show()
    # plt.close()
    # exit()
