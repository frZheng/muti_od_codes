...
if __name__ == "__main__":
    ...
    with open(path + "\odFlow\\" + date +"\part-00000", "r") as fin:
        lines = fin.readlines()
        for line in lines:
            if line[-1] == "\n":
                line = line[:-1]
    ...
    # TaxiDate = date[0:4] + "-" + date[4:6] + "-" + date[6:8] #无分隔符 改为 - 作为分隔符
    TaxiDate = date
    with open(path + "\TaxiODFlow\\" + TaxiDate +"\part-00000", "r") as fin:
        lines = fin.readlines()
        for line in lines:
            if line[-1] == "\n":
                line = line[:-1]
    ...
    plt.savefig("pdf/PubAndFree_{}.pdf".format(date), bbox_inches='tight')#保存到当前文件下savefile.pdf
    # plt.show()
    # plt.close()
    # exit()
